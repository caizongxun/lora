"""
LoRA Model Evaluation - 100 Samples Per Dataset
LoRA 模形評估 - 每个數據集 100 个样本、總計 300 题
"""
...
# Read LoRA model ID from environment or use default
HF_MODEL_ID = os.getenv('HF_MODEL_ID', 'zongowo111/phi3-lora-gsm8k-commonsense')
...
def evaluate_lora_model(base_model_name: str, lora_model_id: str, datasets_dict: Dict):
    """
    Evaluate LoRA model on all datasets
    """
    print("\n" + "="*80)
    print("[LoRA MODEL EVALUATION - 100 SAMPLES PER DATASET]")
    print("="*80)
    
    print_device_info()
    print(f"[INFO] LoRA Model ID: {lora_model_id}")
    ...
    with open('/content/lora/lora_results_100samples.json', 'w') as f:
        json.dump(results_dict, f, indent=2)
    
    print(f"\n[SAVED] Results saved to: /content/lora/lora_results_100samples.json")
...
if __name__ == "__main__":
    print("[INFO] LoRA Model Evaluator - 100 Samples Per Dataset")
