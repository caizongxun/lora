"""
Colab-optimized LoRA Fine-tuning Evaluation Pipeline
...
NOTE: Uses 4-bit quantization for memory efficiency
      This allows the pipeline to run on Colab's free GPU (T4/A100)
"""
...
def setup_colab_output_dir():
    """Setup output directory for Colab (use /content for easy download)."""
    output_dir = "/content/lora_evaluation_results"
...
def main():
    ...
    print("="*80)
    print("[INFO] Using 4-bit quantization for memory efficiency (Colab GPU compatible)")
    print(f"[INFO] LoRA Model ID: {HF_MODEL_ID}")
    print(f"[INFO] Running on Colab")
    print("")
    
    try:
        # Setup Colab output directory
        colab_output_dir = setup_colab_output_dir()
        print(f"[INFO] Output directory: {colab_output_dir}")
        print("")
        ...
        # Sample size configuration for Colab
        # 5: Very quick test (fast, ~3 minutes)
        # 10: Balanced (medium speed, ~10-15 minutes)
        # 50: More comprehensive (slower, ~30+ minutes)
        ...
        print(f"[INFO] All results saved to: {colab_output_dir}/")
        print(f"[INFO] You can download the results from Colab's file explorer")
        print("="*80)
