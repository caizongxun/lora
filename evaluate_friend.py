"""
Standalone LoRA Evaluation Script for Friends
Automatically downloads trained LoRA weights from HuggingFace Hub
and compares Base vs LoRA model performance.

Usage:
    python evaluate_friend.py --hf_model_id caizongxun/phi3-lora-gsm8k-commonsense

Or set environment variable:
    export HF_MODEL_ID=caizongxun/phi3-lora-gsm8k-commonsense
    python evaluate_friend.py
"""
...
class LoraEvaluator:
    ...
    def setup_models(self):
        ...
        if self.hf_model_id:
            ...
            print("⚠️  No LoRA model ID provided, will only evaluate base model")
            self.lora_model = None
    ...
    def evaluate_gsm8k(self, dataset, model) -> Tuple[int, int]:
        ...
        for i, sample in enumerate(tqdm(dataset, desc="GSM8K")):
            ...
            # Simple check: if the predicted answer contains the true answer
    ...
    def evaluate_commonsenseqa(self, dataset, model) -> Tuple[int, int]:
        ...
        for i, sample in enumerate(tqdm(dataset, desc="CommonsenseQA")):
            ...
            ground_truth = choices[int(sample["answerKey"][0], 36) - 10]  # A=10, B=11, etc.
            
            # Generate response
            prompt = f"<|user|>\n{question}\nChoices: {', '.join(choices)}<|end|>\n<|assistant|>\n"
            response = self.generate_response(prompt, model, max_length=128)
            
            # Check if any choice is in the response
    ...
    def evaluate_svamp(self, dataset, model) -> Tuple[int, int]:
    ...
    def evaluate(self, datasets_dict: Dict):
        ...
        for dataset_name, dataset in datasets_dict.items():
            ...
            base_correct, base_total, base_acc = self.evaluate_gsm8k(dataset, self.base_model) if dataset_name == "gsm8k" else \
                                                  self.evaluate_commonsenseqa(dataset, self.base_model) if dataset_name == "commonsenseqa" else \
                                                  self.evaluate_svamp(dataset, self.base_model)
            
            # Evaluate LoRA model (if available)
            lora_acc = None
            if self.lora_model:
                print(f"\n[2/2] Evaluating LoRA Model...")
                lora_correct, lora_total, lora_acc = self.evaluate_gsm8k(dataset, self.lora_model) if dataset_name == "gsm8k" else \
                                                      self.evaluate_commonsenseqa(dataset, self.lora_model) if dataset_name == "commonsenseqa" else \
                                                      self.evaluate_svamp(dataset, self.lora_model)
...
def main():
    ...
    parser.add_argument(
        ...
        help="HuggingFace Hub ID of trained LoRA model (e.g., username/phi3-lora-xxx)"
    )
    parser.add_argument(
        "--base_model",
        type=str,
    ...
    if not args.hf_model_id:
        ...
        print("\nUsage:")
        print("   python evaluate_friend.py --hf_model_id username/phi3-lora-xxx")
        print("\nOr set environment variable:")
        print("   export HF_MODEL_ID=username/phi3-lora-xxx")
        print("   python evaluate_friend.py")
        sys.exit(1)
