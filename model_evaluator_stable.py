"""
Stable Model Evaluator - Production Ready
- No bitsandbytes dependency (fp16 only)
...
def load_base_model(model_name: str):
    """
    Load base model with fp16 (skip 4-bit to avoid bitsandbytes issues)
    """
    print(f"[INFO] Loading {model_name}...")
    print("[INFO] Using fp16 precision (skip 4-bit to avoid compatibility issues)")
    
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        dtype=torch.float16,  # Use new parameter name
        device_map="auto",
...
def generate_response(model, tokenizer, prompt: str, max_tokens: int = 1024) -> str:
    ...
    try:
        ...
        print(f"  [Error generating response: {str(e)[:50]}]")
        return ""
...
def evaluate_on_dataset(model, tokenizer, questions: List[str], ground_truths: List[str]) -> Dict:
    ...
    for i, (q, gt) in enumerate(zip(questions, ground_truths)):
        try:
            ...
            print(f"  Sample {i+1}/{total}: ✗ (Error: {str(e)[:30]})")
...
if __name__ == "__main__":
    print("[INFO] Stable Model Evaluator - Production Ready (fp16 only, no bitsandbytes)")
