...
def get_quantization_config():
    """Create 4-bit quantization config with improved precision"""
    return BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_compute_dtype=torch.float32,  # ✓ IMPROVED: float32 instead of float16
...
def train_lora_model():
    """Main training function for LoRA fine-tuning (IMPROVED VERSION)"""
    print("\n" + "="*80)
    print(" "*20 + "🚀 LoRA Fine-Tuning Training Script (IMPROVED)")
    print("="*80)
    ...
    try:
        # ====================================================================
        # STEP 1: Load Datasets (IMPROVED: 500 samples instead of 100)
        # ====================================================================
        print("\n" + "="*80)
        print("STEP 1: LOADING DATASETS FOR TRAINING")
        print("="*80)
        
        # ✓ IMPROVED: Increased from 100 to 500 samples per dataset
        datasets_dict = load_all_datasets(num_samples=500)
        ...
        # ✓ IMPROVED: Better LoRA configuration
        ...
        # ✓ IMPROVED: Create validation dataset
        val_dataset = train_dataset.select(range(min(len(train_dataset) // 10, 50)))  # 10% validation
        print(f"\n✅ Validation dataset created: {len(val_dataset)} samples")
        
        # ====================================================================
        # STEP 5: Configure Training Arguments (IMPROVED)
        # ====================================================================
        print("\n" + "="*80)
        ...
        # ✓ IMPROVED: Better training parameters
        training_args = TrainingArguments(
            ...
            # Optimization parameters (IMPROVED)
            learning_rate=5e-4,                    # ✓ Increased from 2e-4 to 5e-4
            ...
            # Logging and evaluation (IMPROVED)
            ...
            # Save strategy (IMPROVED)
            save_strategy="epoch",
            save_total_limit=5,                    # ✓ Increased from 2 to 5
            load_best_model_at_end=True,           # ✓ Load best model
            metric_for_best_model="eval_loss",    # ✓ Use eval loss for selection
        ...
        print("="*80)
        print(f"\n⏱️  Estimated training time: 60-120 minutes on RTX 3050 Ti")
        print(f"📊 Checkpoint will be saved every epoch to: {LORA_CHECKPOINT_DIR}")
        print(f"✅ Validation will occur every epoch")
        ...
        print(f"\n🎯 Next Steps:")
        print(f"   1. Run: python evaluate_lora_100samples.py")
        print(f"   2. Or run: python evaluate_baseline_100samples.py for comparison")
        print(f"\n✨ Your improved LoRA model is now trained and ready for evaluation!")
...
if __name__ == "__main__":
    print("\n" + "#"*80)
    print("# LoRA FINE-TUNING TRAINING SCRIPT (IMPROVED VERSION)")
    print("# Fixes: Better config, more data, longer training, validation, early stopping")
    print("#"*80 + "\n")
