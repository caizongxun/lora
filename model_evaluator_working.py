"""
Working Model Evaluator - Fully Fixed
- Correct max_new_tokens usage
...
def generate_response(model, tokenizer, prompt: str, max_tokens: int = 1024) -> str:
    ...
    FIXED: Use max_new_tokens instead of max_length
    """
    try:
        inputs = tokenizer(prompt, return_tensors="pt").to(device)
        
        # Use max_new_tokens properly
        with torch.inference_mode():
            output = model.generate(
                input_ids=inputs['input_ids'],
                attention_mask=inputs.get('attention_mask'),
                max_new_tokens=min(max_tokens, 512),  # Limit to 512 for safety
                num_beams=1,
...
if __name__ == "__main__":
    print("[INFO] Working Model Evaluator - max_new_tokens Fixed")
