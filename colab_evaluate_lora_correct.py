"""
Colab-Optimized LoRA vs Base Model Evaluation Script - CORRECT VERSION
Follows HuggingFace official guidelines: NO device_map, NO .to()/.cuda() calls

Usage in Colab:
    !python colab_evaluate_lora_correct.py --hf_model_id zongowo111/phi3-lora-gsm8k-commonsense
"""
...
class ColabLoRAEvaluator:
    ...
    CORRECT: No device_map, no .to()/.cuda() calls
    """
    ...
    def __init__(
        ...
        print("\n" + "="*80)
        print("🚀 LoRA vs Base Model Evaluation (CORRECT VERSION)")
        print("="*80)
        print(f"\n📊 Configuration:")
    ...
    def setup_models(self):
        ...
        # Load base model
        # CORRECT: No device_map, no .to()/.cuda() calls
        # BitsAndBytes will automatically place model on GPU
        print(f"\n⬇️ Loading base model: {self.base_model_name}...")
        print("   (No device_map - BitsAndBytes handles device placement)")
    ...
    def evaluate_gsm8k(self, dataset, model) -> float:
        ...
        for i, sample in enumerate(tqdm(dataset[:total], desc="GSM8K", leave=False)):
            try:
                ...
                # CORRECT: Move inputs to GPU without calling .to() on model
                # The model is already on GPU via BitsAndBytes
